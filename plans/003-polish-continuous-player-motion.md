...
- Artwork keeps constant geometry across pause/resume. Playback state must not
  add a second transform on top of the full-surface presentation drag.
...
5. In `NowPlayingScreen`, keep artwork at one stable size across playback-state
   changes; `bfdee2f` supersedes the earlier `0.86` pause-scale experiment after
   frame analysis showed it as a second, conflicting transform.
...
- **Feel check**: scrub rapidly and re-grab within 120 ms; the bar never collapses under the finger and surrounding labels do not move. Pause/resume repeatedly; artwork does not resize or pull against the presentation gesture.
