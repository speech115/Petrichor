...
- [ ] Ни одна вьюха не обращается к `databaseManager` (0 вызовов из iOS/ и Views/)
- [ ] Вьюхи не мутируют состояние менеджеров как обходной путь (PlaylistDetailScreen)
- [ ] TrackRow не имеет пути в БД и не держит NSCache
- [ ] Поиск наполняет миниатюры при фильтрации
- [ ] Мёртвые функции базы без вызовов удалены (getTracksByArtist, getAllAlbums и т.п. — список в обзоре)
- [ ] Оба таргета собираются; списки рендерятся с обложками как раньше
