...
- [x] Фаза 1: один `play(track:source:)` и один `isCurrent`; 7+6 копий удалены
- [x] Фаза 2: четыре экрана используют TrackListScreen; ArtistPage/CategoryItemsView — хелпер загрузки
- [x] Фаза 2b: три шапки — один DetailHeader
- [x] Группировка (годы, диски) и пустые состояния не изменились визуально
- [x] Оба таргета собираются; экраны проверены на симуляторе
