...
- [x] `DatabaseManager(pool:)` существует; прод-путь собирается вне менеджера
- [x] Спека-тесты «сборка библиотеки из папки» и «полный импорт M3U» работают
      против in-memory пула и временной папки (раньше были невозможны —
      см. FolderScanTests/PathQueryTests, переписывающие предикаты)
- [x] `resolveTracks(for:)` в Core; политика неоднозначности в одном месте
- [x] Импорт M3U не наполняет обложки сам; вызывающая сторона решает
- [x] LibraryManager и его `fatalError` не тронуты
- [x] Оба таргета собираются; импорт M3U на устройстве работает как раньше
