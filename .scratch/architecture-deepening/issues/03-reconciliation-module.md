...
- [ ] `removeDeletedTracks` и связанные уведомления/счётчики удалены
- [ ] Строки пропавших файлов переживают полный скан (тест на шве: папка →
      удаление файла → скан → строка на месте)
- [ ] iOS-реконсиляция живёт в файле `iOS/`; LMFolders без платформенных веток
- [ ] Триггеры реконсиляции работают (запуск, фон, «Пересканировать»)
