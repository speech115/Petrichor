...
- [x] Все четыре аудита проходят локально и в CI.
- [x] `grep -rn '\.font(\.system(size:' --include='*.swift' iOS | wc -l` = 0
      (либо каждое оставшееся — `@ScaledMetric`-производное, без сырых констант).
- [x] Скриншоты Home, списка треков и Settings на AX5-размере — ничего не
...
- [x] Обе схемы собираются, весь тест-сьют зелёный.
...
- 2026-08-13: влито (PR #6) после rebase на main. VoiceOver на устройстве владелец отказался тестировать; AX-гейт в CI + AX5-скрины закрывают тикет.
