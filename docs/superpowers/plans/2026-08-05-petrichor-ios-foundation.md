...
- Работа идёт в `main` репозитория `~/Projects/apps/petrichor-ios`.
...
Результат записан 2026-08-05. Task 12 принят на реальном iPhone 16 Pro Max.
Отдельная загрузка новой папки через Finder не повторялась: для приёмки использована
уже существующая библиотека на устройстве. Для запуска через обычный device
destination понадобится Xcode с поддержкой iOS 27.0 beta.
